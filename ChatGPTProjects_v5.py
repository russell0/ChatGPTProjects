...
Batch mode (run instructions from a file):
  python ChatGPTProjects_v5.py my_instructions.txt
"""
...
# GPT-5 limits
MAX_INPUT_TOKENS = 272_000   # maximum input tokens
MAX_TOTAL_TOKENS = 400_000   # input + output tokens combined
...
def main():
    ...
    if len(sys.argv) > 1:
        # Support a common form: python ChatGPTProjects_v5.py my_instructions.txt
        # Additional flags could be added later, but for now it's a simple path.
        ...
        try:
            ...
            # Ensure settings (like last project) persist after batch run
            try:
                cli.do_exit("")
            except Exception:
                pass
