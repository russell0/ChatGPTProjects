"""
ChatGPT Projects CLI - Website-Parity Coding Companion (Enhanced, v5)
====================================================================

Feature highlights (Claude-style paradigm, adapted for ChatGPT/GPT models):
- Project isolation: add/remove/list files in a per-project sandbox (ChatGPT sees ONLY current project files)
...
- Model management: gpt-5 (default), gpt-4.1, gpt-4o, gpt-4o-mini; live `models` list from API
- Streamed responses (Responses API if available) with **automatic non-stream fallback** when streaming is not allowed
- Chat Completions fallback if Responses API is unavailable
- Token & cost estimation (tiktoken optional); pricing table is easy to update
...
Hardening (no more 400s from params/org restrictions):
- Do **not** send `temperature` to Responses API (some models reject it)
- If streaming is **not allowed** for your org/model, auto-retry **non-streaming** and print the full text
- Retain streaming for Chat Completions (where supported)
...
# --- OpenAI client bootstrap (Responses API preferred) ---
try:
...
PRICING_USD_PER_1K: Dict[str, Dict[str, float]] = {
    "gpt-5":        {"input": 0.00, "output": 0.00},  # TODO: update when official
...
TOKEN_LIMIT = 200_000
SYSTEM_INSTRUCTIONS = (
    "You are ChatGPT, an elite software engineer and assistant."
...
class Project:
    ...
    def __init__(self, name: str, base_path: Optional[Path] = None):
        self.name = name
        self.base_path = base_path or Path.cwd() / "ChatGPT_Projects"
...
class GPTClient:
    ...
    def responses_api_supported(self) -> bool:
        return hasattr(self.client, 'responses') and hasattr(self.client.responses, 'create')
    ...
    def _extract_responses_text(self, resp: Any) -> str:
        """
        Be liberal in what we accept; different SDKs/versions expose text differently.
        """
        # Newer SDKs provide .output_text
        if hasattr(resp, "output_text") and isinstance(resp.output_text, str):
            return resp.output_text

        # Some expose a list in .output
        out = getattr(resp, "output", None)
        if isinstance(out, list):
            chunks = []
            for item in out:
                # Each item may have .content which is a list of blocks
                content = item.get("content") if isinstance(item, dict) else getattr(item, "content", None)
                if isinstance(content, list):
                    for block in content:
                        # Favor 'output_text' or 'text' keys
                        if isinstance(block, dict):
                            if "output_text" in block and isinstance(block["output_text"], str):
                                chunks.append(block["output_text"])
                            elif "text" in block and isinstance(block["text"], str):
                                chunks.append(block["text"])
                        else:
                            maybe = getattr(block, "text", None)
                            if isinstance(maybe, str):
                                chunks.append(maybe)
            if chunks:
                return "".join(chunks)

        # Fallback: try generic .text
    ...
    def stream(self, *, model: str, system: str, user: str, temperature: Optional[float] = 0.2):
        """
        Yield text deltas as they arrive (Responses API preferred).

        Robust behavior:
        - Do NOT send 'temperature' to Responses API
        - Try streaming; if 400 due to org/model not allowed to stream, retry non-streaming and yield once
        - If Responses API fails for content-type reasons, retry 'instructions' variant
        - If still failing, fall back to Chat Completions (streaming) with temperature allowed
        """
        # --- Use Responses API when available ---
        if self.responses_api_supported():
            # 1) Primary attempt: streaming via Responses API (no temperature)
            try:
                req_args = {
                    "model": model,
                    "input": [
                ...
                stream = self.client.responses.create(**req_args)
                for ev in stream:
                    if ev.type == "response.output_text.delta":
                        yield ev.delta
                    elif ev.type == "response.completed":
                        break
                return  # done if streaming worked

            except Exception as e_stream:
                msg = str(e_stream)
                # If org/model not allowed to stream: retry non-streaming
                if ("param': 'stream'" in msg) or ("must be verified to stream" in msg) or ("unsupported_value" in msg):
                    try:
                        req_args = {
                            "model": model,
                            "input": [
                        ...
                        # Try 'instructions' non-streaming as a final Responses attempt
                        try:
                            req_args = {
                                "model": model,
                                "instructions": system,
                    ...
                    # If it's a type/role issue, try 'instructions' streaming first
                    if ("input[0].content[0].type" in msg) or ("invalid_value" in msg):
                        try:
                            req_args = {
                                "model": model,
                                "instructions": system,
                            ...
                            try:
                                ...
                                pass
                    # Any other Responses error => fall back to Chat Completions
                # If we reach here, we'll try Chat Completions next

        # 2) Fallback: Chat Completions (streaming) - temperature allowed here
        try:
            ...
            for ch in stream:
                d = getattr(getattr(ch, "choices", [None])[0], "delta", None)
                if d and getattr(d, "content", None):
                    yield d.content
        except Exception as e:
            # As a last resort, do a non-streaming single call to Chat Completions
            try:
                ...
                if hasattr(comp, "choices") and comp.choices:
                    ...
                    text = getattr(msg, "content", "") or (msg.get("content") if isinstance(msg, dict) else "")
                if text:
                    yield text
                else:
                    raise RuntimeError("Empty response from Chat Completions.")
            except Exception as e2:
                raise RuntimeError(f"Chat Completions API error: {e2}")

# =====================
# CLI
# =====================
class ChatGPTCLI(cmd.Cmd):
    intro = (
        "\n" +
        "╔" + "═"*63 + "╗\n" +
        "║                 ChatGPT Projects CLI (Enhanced)                 ║\n" +
        "║  Manage isolated project files and chat with GPT about them     ║\n" +
        "╠" + "═"*63 + "╣\n" +
        "║  📁 Projects are isolated - ChatGPT sees ONLY current project    ║\n" +
        "╚" + "═"*63 + "╝\n\n" +
    ...
    def __init__(self):
        ...
        try:
            ...
            print(f"⚠️  Warning: {e}")
            print("   Create a .env with OPENAI_API_KEY=... or export it in your shell.")
        self.current_project: Optional[Project] = None
        self.projects_base = Path.cwd() / "ChatGPT_Projects"
        self.projects_base.mkdir(exist_ok=True)
        self.settings_file = self.projects_base / ".settings.json"

        settings = self._load_settings()
        self.current_model_key = settings.get('model_key', 'gpt5')

        # Re-open last project if available
        last_project = settings.get('last_project')
        if last_project:
            maybe = self.projects_base / last_project
            if maybe.exists():
                self.current_project = Project(last_project)
                self.prompt = f'({last_project}) > '
                print(f"✅ Reopened last project: {last_project}")
    ...
    def _save_settings(self) -> None:
        ...
        if self.current_project:
            data['last_project'] = self.current_project.name
    ...
    def do_tokens(self, _):
        ...
        print("-"*65)
        overhead = 2_000
        print(f"TOTAL with ~overhead: {(total+overhead):,} / {TOKEN_LIMIT:,}")
    ...
    def do_model(self, name: str):
        """Switch model by alias or exact id: model <gpt5|4.1|4o|mini|model-id>"""
        if not name:
    ...
    def do_pricing(self, args: str):
        """Estimate one-shot and hourly cost.
Usage: pricing --model <id|alias> --estimate-tokens <int> --in-ratio <0..1> --throughput-tps <float>
Defaults: --in-ratio 0.5  --throughput-tps 30
"""
        argv = args.split()
        opt = {"--model": None, "--estimate-tokens": "200000", "--in-ratio": "0.5", "--throughput-tps": "30"}
        for i in range(0, len(argv), 2):
            if i+1 < len(argv) and argv[i] in opt:
                opt[argv[i]] = argv[i+1]
        model_in = opt["--model"] or self.MODELS.get(self.current_model_key, self.current_model_key)
        try:
            total = int(opt["--estimate-tokens"])
            in_ratio = float(opt["--in-ratio"])
            tps = float(opt["--throughput-tps"])
        except Exception:
            print("❌ Bad args. See 'help pricing'")
            return
        in_tokens = max(0, int(total*in_ratio))
        out_tokens = max(0, total - in_tokens)
        pricing = PRICING_USD_PER_1K.get(model_in)
        if not pricing:
            print(f"❌ No pricing entry for '{model_in}'. Update PRICING_USD_PER_1K in script.")
            return
        cost = (in_tokens/1000.0)*pricing['input'] + (out_tokens/1000.0)*pricing['output']
        hourly = (tps*3600.0) * (0.5*(pricing['input']+pricing['output'])/1000.0)
        print(f"Model: {model_in}\nEstimated tokens: {total:,} (in {in_tokens:,} / out {out_tokens:,})\nOne-shot cost: ${cost:,.2f}\nEstimated ${hourly:,.2f}/hr @ {tps} tok/s")

    # =========================
    # Chat (with strict project scope)
    # =========================
    def do_chat(self, message: str):
        """Chat with ChatGPT about your project files.
...
"""
        if self.client is None:
            print("❌ No OpenAI client. Configure OPENAI_API_KEY.")
            return
        if not message:
            ...
            if selected_files:
                ...
                for fn in selected_files:
                    try:
                        ...
                        if context_tokens + tok + self.estimate_tokens(message) > TOKEN_LIMIT - 5_000:
                            print(f"⚠️  Skipping {fn} (would exceed token budget)")
                            continue
                ...
                if context_tokens + self.estimate_tokens(message) > TOKEN_LIMIT - 5_000:
                    print(f"⚠️  Project files exceed token budget (~{context_tokens:,})")
        ...
        print(f"📊 Estimated tokens: {total_est:,} / {TOKEN_LIMIT:,}")
        if total_est > TOKEN_LIMIT - 1_000:
            print("❌ Request too large. Reduce context or message size.")
            return
        ...
        system = SYSTEM_INSTRUCTIONS + " " + GPTClient.WEBSITE_STYLE

        print("🤔 Thinking (streaming if allowed)...")
        response_text = []
        try:
            for delta in self.client.stream(model=model_id, system=system, user=user_text, temperature=0.25):
        ...
        if self.current_project:
            ...
            artifacts = am.extract_code_blocks(full)
            if artifacts:
        ...
        print(f"   • Words: {len(full.split())} words")
...
"""
    ...
    def do_help(self, arg):
        ...
        print(
            """
🎯 ChatGPT Projects CLI - Command Reference
==========================================
PROJECTS:
...
"""
    ...
    def do_exit(self, _):
        # Persist settings (including last project) on exit
        self._save_settings()
        print("👋 Goodbye!")
        return True
...
def main():
    print("\n🚀 Starting ChatGPT Projects CLI (Enhanced)\n")
    cli = ChatGPTCLI()
    try:
        cli.cmdloop()
    except KeyboardInterrupt:
